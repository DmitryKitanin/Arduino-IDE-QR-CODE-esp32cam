// https://github.com/DmitryKitanin/Arduino-IDE-QR-CODE-esp32cam
// 2021, Dmitry Kitanin
...
{
  quirc_decode_error_t err;
  struct datastream *ds = (datastream*)ps_malloc(sizeof(struct datastream));
...
struct quirc *quirc_new(void)
{
  struct quirc *q = (quirc*)ps_malloc(sizeof(*q));
...
int quirc_resize(struct quirc *q, int w, int h)
{
  ...
  uint8_t *new_image = (uint8_t*)ps_malloc(w * h);
  ...
  { 
    ...
    if (q->pixels)
      free(q->pixels);
    quirc_pixel_t *new_pixels =(quirc_pixel_t*) ps_malloc(new_size);
    if (!new_pixels)
    {
      free(new_image);
      return -1;
